...
class BaseDAO extends PDO
{
	...
	{
		$dsn = $this->createdsn($this->options);
		$attrs = empty($this->options['charset']) ? array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES " . $this->charset) : array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES " . $this->options['charset']);
		try
		{
